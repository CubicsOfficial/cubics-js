...
export default {
    ...
    async generatePublic(privateKey: string): Promise<string> {
        return Utils.base58encode(
            await ed.getPublicKey(Utils.base58decode(privateKey)))
    },
    async generateKeypair(): Promise<string[]> {
        var privateKey = this.generatePrivate()
        return [await this.generatePublic(privateKey), privateKey]
    },
     ...
     */
    async pbkdf2(message, salt=null) {
        var crypto = this.crypto()
